%% CALLPRICE - Call option price by orthogonal expansions
% C=CallPrice(m,IncMomA,IncMomB,bt,K,r,T)
% 
% Computes the price C of an european call option by expanding
...
%
% Output:                                                                
%         C -  MAT: approximated integral of P wtr to the real measure of
...
function C=CallPrice(c,I,IncMom,a,K)
...
for k=0:n-1                              % evaluation of A(k)
    for i=0:k
        A(k+1)=A(k+1)+a(k+1,i+1)*(I(i+1)-K*IncMom(i+1));
    end
end
%c*gamma(alpha+1)
